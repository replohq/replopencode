...
| `lazy`        | Boolean |          | Defer connecting until a tool is needed. See [Lazy loading](#lazy-loading) below.        |
...
By default opencode connects to every configured MCP server on startup and loads all
of their tools into the model's context. With many servers — or servers that are slow
to start — this delays your first response and consumes a large chunk of the context
window.

Set `lazy` to `true` to defer a server. A lazy server is **not** connected at startup
and its tools are **not** loaded upfront. Instead, opencode exposes a single `mcp` tool
that the model uses to discover and load a server's tools on demand. When the model
loads a server, opencode connects to it and its tools become available on the next step.
...
To defer every server by default, set `experimental.mcp_lazy` to `true`. Individual
servers can still opt back in by setting `lazy: false`.
...
To keep discovery fast, opencode caches each server's tool catalog to disk after the
first successful connection, so a lazy server can advertise its tools on later startups
without connecting. The cache is keyed by the server's connection settings and refreshed
